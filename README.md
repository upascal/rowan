...
A task management application that provides a **bidirectional link** between Markdown task lists and a visual Kanban board. It allows you to seamlessly switch between editing your tasks in a standard Markdown file and managing them visually on a Kanban board. Changes made in one view are automatically reflected in the other, keeping your Markdown file and Kanban board perfectly synchronized. This is ideal for integrating Markdown-based task lists (like those generated by AI tools) into a visual workflow, while still maintaining the ability to edit the underlying Markdown directly. Available as both a Progressive Web App (PWA) and a macOS application.

## Features

- **Markdown to Kanban**: Automatically converts Markdown headings to columns and list items to cards
- **Bidirectional Real-time Sync**: Changes made either in the Markdown editor or on the Kanban board (e.g., editing text, checking boxes, dragging cards) are immediately reflected in the other view.
